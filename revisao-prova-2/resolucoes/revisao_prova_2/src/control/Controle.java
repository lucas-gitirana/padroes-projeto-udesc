...
import model.Card;
import visitor.VisitorContaPares;
import visitor.VisitorTrocaImagens;
...
public class Controle {
	...
	public void contouPares() {
		VisitorContaPares visitorContaPares = new VisitorContaPares();
		
		for (Card c : cards) {
			c.accept(visitorContaPares);
		}
		
		for (Observer obs:obss) {
			obs.exibeContagem(visitorContaPares.getParesCorretos());
		}
	}

	public void trocouImagens() {
		for (Card c : cards) {
			VisitorTrocaImagens visitorTrocaImagens = new VisitorTrocaImagens();
			c.accept(visitorTrocaImagens);
		}
	}
}
